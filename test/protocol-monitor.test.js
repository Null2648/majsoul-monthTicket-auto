...
function makeProtocol() {
  ...
  const methods = {
    ...
    Lobby: {
      login: ['ReqLogin', 'ResLogin'], oauth2Check: ['ReqOauth2Check', 'ResOauth2Check'],
      oauth2Auth: ['ReqOauth2Auth', 'ResOauth2Auth'], oauth2Login: ['ReqOauth2Login', 'ResLogin'],
      payMonthTicket: ['ReqCommon', 'ResPayMonthTicket'], fetchMonthTicketInfo: ['ReqCommon', 'ResMonthTicketInfo'],
      gainReviveCoin: ['ReqCommon', 'ResCommon'], fetchShopInfo: ['ReqCommon', 'ResShopInfo'],
  ...
  return {
    nested: { lq: { nested: {
      Route: service(methods.Route), Lobby: service(methods.Lobby),
      ...
      ReqCommon: { fields: {} }, ResCommon: { fields: { error: commonError } },
      ResPayMonthTicket: { fields: { error: commonError } }, ResMonthTicketInfo: { fields: { error: commonError } },
...
test('unrelated optional additions only warn about the raw protocol change', () => {
  const before = buildProtocolSnapshot(makeProtocol());
  const changed = makeProtocol();
...
test('compatibility patch does not mutate or hide the raw official protocol', () => {
  const protocol = makeProtocol();
  delete protocol.nested.lq.nested.ReqRequestConnection.fields.platform;
  const raw = JSON.stringify(protocol);
  const snapshot = buildProtocolSnapshot(protocol);
  assert.equal(JSON.stringify(protocol), raw);
  assert.equal(snapshot.compatibilityPatches.length, 1);
  assert.equal(snapshot.compatibilityPatches[0].path, 'lq.ReqRequestConnection.platform');

  const restored = makeProtocol();
  const report = compareProtocolSnapshots(snapshot, buildProtocolSnapshot(restored));
  assert.ok(report.breaking.some(value => value.includes('compatibility patches changed')));
});

test('required fields in a reused message type are tracked under every path', () => {
  const root = { nested: { lq: { nested: {
    Req: { fields: { left: field('Shared', 1), right: field('Shared', 2) } },
    Shared: { fields: { proof: field('string', 1, 'required') } }
  } } } };
  const paths = collectRequiredRequestPaths(root, 'lq.Req').map(item => item.path);
  assert.deepEqual(paths, ['left.proof', 'right.proof']);
});
