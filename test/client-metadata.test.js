...
test('parseUnityBuildId reads the exact official Unity loader build', () => {
  const html =
    '<script src="Build/jp-WebGL-release-4.0.11(12).loader.js"></script>';

  assert.equal(parseUnityBuildId(html), 'jp-WebGL-release-4.0.11(12)');
});

test('parseUnityBuildId rejects pages without an official Unity build asset', () => {
  assert.throws(
    () => parseUnityBuildId('<html></html>'),
    /Unity build identifier not found/
  );
});
