...
8. 예약 출석은 `Asia/Seoul` 기준 오전 6시 17분부터 10시 17분까지 매시간 시도하며, 오후 12시 13분에 최종 복구를 실행합니다. 한 번 성공하면 같은 날의 후속 실행은 설치와 로그인을 시작하기 전에 종료됩니다.
9. 별도 `Attendance Watchdog`은 오전 11시 31분과 오후 1시 31분에 성공 기록을 확인하고, 미완료 상태라면 신뢰된 `main`의 출석 워크플로를 다시 호출합니다.
10. 저장소 상단 `Actions` 탭에서 `Login to Majsoul`과 `Attendance Watchdog`을 각각 활성화합니다.

## 수동 실행 방법
1. `Actions > Login to Majsoul`을 엽니다.
2. `Run workflow`를 누릅니다.
3. **`Use workflow from`은 반드시 `main`을 선택합니다.** 다른 브랜치를 선택하면 계정 Secret 보호를 위해 출석은 실행되지 않으며, 실행 화면에 원인이 표시됩니다.
4. 오늘 이미 성공했어도 다시 확인하려면 `오늘 이미 성공했어도 다시 실행`을 켠 상태로 실행합니다.
5. 정상 실행 시 `attendance-run-report.json` 진단 파일과 단계별 실행 요약을 확인할 수 있습니다. 성공하면 브라우저 세션이 중복 로그인으로 종료될 수 있습니다.
...
## 주의
- GitHub Actions 예약 실행은 서버 부하에 따라 지연되거나 개별 이벤트가 누락될 수 있습니다. 오전 다중 예약과 독립 감시 워크플로가 이를 보완하지만 정확한 분 단위 시작은 보장되지 않습니다.
